...
Datfile = namedtuple('Datfile', ['file','program1','program2','program3','program4','stat1','stat2','stat3','stat4','perbyte','rows'])
		...
		for datrow in datfile.rows:
			...
			for row in incsv:
				for fileexpression in datrow.fileexpressions:
					if row.expression==fileexpression.expression and row.file == fileexpression.file and ((row.program==datfile.program1 and row.stat==datfile.stat1) or (row.program==datfile.program4 and row.stat==datfile.stat4)):
						value1=value1*float(row.value)
						valuecount1=valuecount1+1
					if row.expression==fileexpression.expression and row.file == fileexpression.file and ((row.program==datfile.program2 and row.stat==datfile.stat2) or (row.program==datfile.program3 and row.stat==datfile.stat3)):
						value2=value2*float(row.value)
